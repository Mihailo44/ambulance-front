...
class MedicalEvaluatorRegistration extends StatefulWidget {
  ...
  const MedicalEvaluatorRegistration({super.key});
        ...
        child: Center(
            ...
            child: Column(
              children: [
                ...
                buildTextFormField(controller: _firstnameController, labelText: "Firstname"),
                const SizedBox(height: 15.0,),
                buildTextFormField(controller: _lastnameController, labelText: "Lastname"),
                const SizedBox(height: 15.0,),
                buildTextFormField(controller: _passwordController, labelText: "Password"),
                const SizedBox(height: 15.0,),
                buildTextFormField(controller: _dateOfBirthController, labelText: "Date Of Birth"),
                const SizedBox(height: 15.0,),
                ElevatedButton(
                  onPressed: () async{
                    final OperatingBase selectedBase = await Navigator.push(
                      context, 
                      MaterialPageRoute(builder: (context) => AllOperatingBases())
                    );
                ...
                ElevatedButton(
                  onPressed: (){
                    User user = User(firstname: _firstnameController.value.text, 
                                    lastname: _lastnameController.value.text, 
                                    username: "username", password: _passwordController.value.text, 
                                    dateOfBirth: DateTime(2021,9,7,17,30), //DateTime.parse(_dateOfBirthController.value.text),
                                    role: UserRole.MEDICAL_EVALUATOR);
                    MedicalEvaluator evaluator = MedicalEvaluator(userId: -1, user: user, operatingBaseId: _selectedOperatingBase!.id, operatingBase: _selectedOperatingBase!);
                    showSnackBar(context, _selectedOperatingBase!.address.city);
                  }, 
